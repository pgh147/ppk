...
public class LoginController {
    ...
    public String login(@ModelAttribute("userForm") User user, RedirectAttributes redirectAttributes) {
        ...
        if (currentUser.isAuthenticated()) {
            ...
            return "redirect:/product/salesvolume/page";
        } else {
            token.clear();
    ...
    @RequestMapping(value = "/user/updatePassword.json", method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object>  updatePassword(@ModelAttribute("userForm") User user) {
    ...
    @RequestMapping(value = "/user/userInfo.json", method = RequestMethod.GET)
    @ResponseBody
    public Map<String, Object>  userInfo() {
    	Map<String, Object> result = new HashMap<String, Object>();
    	Principal principal = (Principal)SecurityUtils.getSubject().getPrincipal();
    	result.put("data", userService.findUserByName(principal.getUser().getUsername()));
        result.put("retCode", "0");
        return result;
    }
    
    
    @RequestMapping(value = "/user/updateUserInfo.json", method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object>  updateUserInfo(@ModelAttribute("userForm") User user) {
    	Map<String, Object> result = new HashMap<String, Object>();
    	Principal principal = (Principal)SecurityUtils.getSubject().getPrincipal();
    	if(!principal.getUser().getUsername().equals(user.getUsername())){
