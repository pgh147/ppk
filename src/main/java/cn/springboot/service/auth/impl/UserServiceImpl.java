...
public class UserServiceImpl implements UserService {
    ...
    public void updatePassword(User user) {
        ...
        User u = userMapper.findById(principal.getUser().getId());
        if(null == u){
        	throw new BusinessException("1001", "查询用户失败");
        }
        
        String dataBaseOldPassword = u.getPassword();
        System.out.println("dataBaseOldPassword="+dataBaseOldPassword);
        
//        byte[] salt = Digests.generateSalt(SALT_SIZE);
//        user.setSalt(Encodes.encodeHex(salt));
        byte[] salt =Encodes.decodeHex(u.getSalt());
        byte[] hashPassword = Digests.sha1(user.getOldPassword().getBytes(), salt, HASH_INTERATIONS);
        String inputBaseOldPassword = Encodes.encodeHex(hashPassword);
        
        if(!dataBaseOldPassword.equals(inputBaseOldPassword)){
            throw new BusinessException("1002", "用户输入的原密码错误");
        }
